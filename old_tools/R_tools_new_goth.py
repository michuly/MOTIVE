...
def vinterp(var, depths, z_r, z_w=None, mask=None,imin=0,jmin=0,kmin=1, floattype=np.float32,interp_sfc=1,interp_bot=0,below=None,**kwargs):
    ...
    if rank(depths)==1: newz = np.asfortranarray(Zeros((z_r.shape[0],z_r.shape[1],len(depths))) + depths, dtype=floattype)
    else: newz = depths
...
def diffy(var,pn,dn=1):
    ...
    if rank(var)<3: dvardy = diffy_2d(var,pn,dn)
...
def vertIntSeamount(var, z_w, z_r, h, hbmax):
    ...
    hmax = np.amax(h)
    if var.shape == z_r.shape:
       Hz = z_w[:,:,1:] - z_w[:,:,:-1]
       cff = z_r + hbmax
       Hz[cff < 0] = 0
    elif var.shape == z_w.shape:
       Hz = z_w*0
       Hz[:,:,1:-1] = z_r[:,:,1:] - z_r[:,:,:-1]
       cff = z_w + hbmax
...
#######################################################
def surfgeo(zeta, gd, g = 9.81):
    fac = g/np.mean(gd['f'])
    u =-v2rho(diffy(zeta,gd['pn']))*fac
    v = u2rho(diffx(zeta,gd['pm']))*fac
    return u, v 
...
def get_pressure_z(b,depths, pm, pn, zeta = None, g = 9.81, rho0 = None):
    p=b*0
    if zeta is None:
        zeta = pm * 0
    if rho0 is None    :
        rho0 = 1
    p[:,:,0]=rho0*g*zeta[:,:]
    p[:,:,1:]=rho0*integrate.cumtrapz(b,depths) + rho0*g*zeta[:,:, None] 
    #print w.shape
    return p
#######################################################
#Shear 2d 
...
def get_pzrressure_z(b,zeta,g,depths,pm,pn,rho0):
    p=b*0
    p[:,:,0]=g*zeta[:,:]
    p[:,:,1:]=g*zeta[:,:,None]+integrate.cumtrapz(b,depths)  
    p=p*rho0
    #print w.shape
    #vrt on psi-rho grid
    return p
...
def shearvd(vd, gd):
    return shear2d(vd['u'], vd['v'], gd['pm'], gd['pn'])

def shear2d_time(u,v,pm=1,pn=1):
    
    if u.shape[0]==v.shape[0]: 
        ux=u2rho_2dp1( diffx_2d_time(u,pm))
        vy=v2rho_2dp1( diffy_2d_time(v,pn))
        uy=v2rho_2dp1( diffy_2d_time(u,pn))
        vx=u2rho_2dp1( diffx_2d_time(v,pm))   
        ...
        ux[1:-1,:,:]=diffx_2d_time(u,rho2u(pm))
        vy[:,1:-1,:]=diffy_2d_time(v,rho2v(pn))
        uy=psi2rho_2dp1( diffy_2d_time(u,rho2u(pn)))
        vx=psi2rho_2dp1( diffx_2d_time(v,rho2v(pm)))
...
def nansub(var,x,y,xsub,ysub):
    ...
    var_bool = inpolygon(x,y,xsub,ysub)
...
def div2uv(u,v, gd):
    ...
    b = -1*div.flatten()*1/(pm.flatten())**2    # right hand side
    ...
    #print('Inside Rtools:', udiv.shape, vdiv.shape) 
    return udiv,vdiv
...
def Basemap_init(x_rho,y_rho,axis=None,proj='merc',font=18,labelx=1,labely=1):
    ...
    lataxis=np.round(np.linspace(np.amin(y_rho),np.amax(y_rho),5))
    lonaxis=np.round(np.linspace(np.amin(x_rho),np.amax(x_rho),5))
...
###################################################################################
def readvars(nch, varlist, time, funclist = None, ij = None):
    vardict = {}
...
def subset(nc, var, ij, itime=None):
    ...
    if xx=='xi_u':
        imin = imin -1
    if itime is None:
        return nc.variables['lon_rho'][...,jmin:jmax,imin:imax] 
    else:
        return nc.variables['lon_rho'][itime,...,jmin:jmax,imin:imax] 
####################################################################################
def ncvarcopy(src, dst, ivarname, varlist, exclude =  None):
...
####################################################################################
def nc1varcopy(src, dst, name, nameout = None, exclude = None):
    ...
    if nameout is not None:
        ...
        name1 = name
    dst.createVariable(name1, ivar.datatype, dims)
    dst[name1].setncatts(src[name].__dict__)
####################################################################################
...
#returns masks for cyclones, anticylones and fronts
####################################################################################
def masks(gradb, vrt, gradval = 4e-8, okubo = None):
    maskgb = gradb * 0
    maskgb[gradb > gradval] = 1
    maskvrtcy = vrt * 0
    maskvrtacy = vrt * 0
    #maskvrtcy[np.logical_and(vrt > 0.0, okubo>0)] = 1
    #maskvrtacy[np.logical_and(vrt < 0.0, okubo>0)] = 1
    maskvrtcy[vrt > 0.05] = 1
    maskvrtacy[vrt < -0.05] = 1
    print(np.sum(maskgb), np.sum(maskvrtcy), np.sum(maskvrtacy))
    return  maskgb, comp(maskgb)*maskvrtcy, comp(maskgb)*maskvrtacy
